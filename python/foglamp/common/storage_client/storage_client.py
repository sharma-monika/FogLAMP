...
class ReadingsStorageClientAsync(StorageClient):
    """ Async wrapper around ReadingsStorageClient """
    _base_url = ""
    ...
    async def append(cls, readings):
        return ReadingsStorageClient.append(readings)

    @classmethod
    async def fetch(cls, reading_id, count):
        return ReadingsStorageClient.fetch(reading_id, count)

    @classmethod
    async def query(cls, query_payload):
        return ReadingsStorageClient.query(query_payload)

    @classmethod
    async def purge(cls, age=None, sent_id=0, size=None, flag=None):
        return ReadingsStorageClient.purge(cls, age, sent_id, size, flag)
