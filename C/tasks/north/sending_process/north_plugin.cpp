...
NorthPlugin::NorthPlugin(const PLUGIN_HANDLE handle) : Plugin(handle)
{
        ...
        pluginInit = (PLUGIN_HANDLE (*)(const string& config))
	...
	pluginSend = (uint32_t (*)(const PLUGIN_HANDLE, const vector<Reading *>))
 ...
 */
PLUGIN_HANDLE NorthPlugin::init(const string& config)
{
...
/**
 * Send vector of readings pointer to historian server
 *
 ...
 */
const string& NorthPlugin::config() const
{
